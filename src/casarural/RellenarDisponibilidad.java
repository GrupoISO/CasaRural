...
public class RellenarDisponibilidad extends JFrame {
	private static final long serialVersionUID = 4987497137295900532L;
	...
	public RellenarDisponibilidad(List<Integer> v) {
		try {
			jbInit(v);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void jbInit(List<Integer> v) throws Exception {
		this.getContentPane().setLayout(null);
		this.setSize(new Dimension(513, 433));
		this.setTitle("Rellenar Disponibilidad");
		jComboBox1 = new JComboBox(v.toArray());
	...
	private void jButton1_actionPerformed(ActionEvent e) {
		...
		// Eliminamos la parte de hora:minuto:segundo:ms de la fecha para
		// normalizarla con lo que aparece en la BD
		...
		// Puede que lance una excepcion en caso de que no se introduzca un
		// numero
		float precio = Float.parseFloat(jTextField3.getText());
		try {
			PantallaInicio.interfazfachada.asignarOferta(numCasa, diaIni,
					diaFin, precio);
