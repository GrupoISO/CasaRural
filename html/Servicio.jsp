...
<%@ page import="casarural.Recogida" %>
		...
		<form action="Servicio2.jsp" name="f" method="post" onSubmit="return validar();">
			...
			<p>
				...
				<select id="recogida" name="recogida">
					<%
						List<Recogida> recogidas = crearServicioBean.getRecogidas();
						if (recogidas != null) {
						for (Recogida r: recogidas) {
					%>
					<option value="<%= r.getNumRecogida() %>"><%= r.getNombre() %></option>
					<%
						}
						}
					%>
